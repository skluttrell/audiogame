...
from lib.sound_pool import sound,sound_positioning
...
class sound_pool:
 ...
 def play_stationary(self,filename,persistent=False,looping=False):
  return self.play_stationary_extended(filename, 0, 0, 0, 100, persistent, looping)
 def play_stationary_extended(self,filename, offset, start_pan,start_volume,start_pitch,persistent=False, looping=False):
  ...
  self.items.append(s)
  return s
 def play_1d(self,filename,listener_x,sound_x,persistent=False,looping=False):
  return self.play_extended_1d(filename, listener_x, sound_x, 0, 0, 0, 0, 0, 100, persistent, looping)
 def play_extended_1d(self,filename,listener_x,sound_x,left_range,right_range,offset,start_pan,start_volume,start_pitch,persistent=False,looping=False):
  self.clean_frequency-=1
  if self.clean_frequency<=0: self.clean_unused()
  ...
  self.items.append(s)
  return s
 def play_2d(self,filename,   listener_x,   listener_y,   sound_x,   sound_y,   persistent=False,   looping=False):
  return self.play_extended_2d(filename, listener_x, listener_y, sound_x, sound_y, 0, 0, 0, 0, 0, 0, 0, 100, persistent, looping)
 def play_extended_2d(self,filename,   listener_x,   listener_y,   sound_x,   sound_y,   left_range,   right_range,   backward_range,   forward_range,   offset,   start_pan,   start_volume,   start_pitch,   persistent=False,   looping=False):
  self.clean_frequency-=1
  if self.clean_frequency<=0: self.clean_unused()
  ...
  self.items.append(s)
  return s
 def play_3d(self,filename,   listener_x,   listener_y,   listener_z,   sound_x,   sound_y,   sound_z,   persistent=False,keep_pitch=False,   looping=False):
  return self.play_extended_3d(filename, listener_x, listener_y, listener_z, sound_x, sound_y, sound_z, 0, 0, 0, 0, 0, 0, 0, 0, 0, 100, persistent,keep_pitch, looping)
 ...
 def clean_unused(self):
  ...
  for i in self.items:
    ...
    #speak("Looping, "+str(i.looping)+" persistent, "+str(i.persistent)+", handle "+str(i.handle.handle)+", playing "+str(i.handle.handle.is_playing)+", paused"+str(i.paused)+". ",False)
   #else: speak("This is none! Hopefully it will get cleaned up.")
